...
use Masteriyo\ShowHideComponents\ShowHideCourseListComponents;
...
class Masteriyo {
	 ...
	 * Fire before masteriyo is initialized.
		...
		AdminMenu::init();
		ScriptStyle::init();
		ShowHideCourseListComponents::init();
	...
	public function masteriyo_email_verification_handler() {
		...
		if ( ! masteriyo_is_account_page() || ! isset( $_GET['token'] ) || ! isset( $_GET['uid'] ) || ! isset( $_GET['nonce'] ) ) {
			return;
		}
	...
	public static function resend_email_verification_email_handler() {
		if ( ! masteriyo_is_account_page() || ! isset( $_GET['uid'] ) || ! isset( $_GET['resend_email_verification'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			return;
		}
